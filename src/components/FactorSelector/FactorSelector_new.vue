    ...
    <div>
        <a-row v-if="factorSelectShow">
          <a-col :span="12" :sm="13" :xs="24">
            ...
            <div class="factor-row" v-show="row3.length">
              <a-checkbox-group :options="row3" :value="checkedValues" @change="onRow3Change" />
            </div>
            <div class="date-row">
              参考日期：
              <a-date-picker @change="onDateChange" style="margin-right:60px"/> <a-button ref="searchBtn" type="primary" @click="onSearch">查询</a-button>
            </div>
          </a-col>
...
export default {
  ...
  computed: {
    ...
    row3 () {
      ...
      if (this.factors[this.row1Active]) {
        let arr = this.factors[this.row1Active].children[this.row2Active].children
        if (arr.length) {
  ...
  methods: {
    ...
    getFactors () {
      monitor.getFactorFilterItem({ measurePointId: this.pointId })
        .then((res) => {
    ...
    getFilterItem () {
      ...
      if (this.row2[this.row2Active].children.length > 0) {
        filterItem.locationArr = this.checkedValues.join(',')
      } else {
        filterItem.locationArr = this.row2[this.row2Active].fullKey
      }
      return filterItem
    },
    onSearch () {
