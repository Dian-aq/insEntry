    ...
    <a-form ref="formRegister" :form="form" id="formRegister">
      ...
      <a-form-item>
        <a-button
          ...
          :loading="registerBtn"
          @click.stop.prevent="registerSubmit"
    ...
    registerSubmit () {
      ...
      validateFields({ force: true }, (err, values) => {
        if (!err) {
          ...
          $router.push({ name: 'registerResult', params: { ...values } }).catch(() => {})
        }
      })
    },
      ...
      validateFields(['mobile'], { force: true },
        (err, values) => {
          if (!err) {
            ...
            login.getSmsCaptcha({ mobile: values.mobile }).then(res => {
              setTimeout(hide, 2500)
              $notification['success']({
                message: '提示',
                description: '验证码获取成功，您的验证码为：' + res.code,
                duration: 8
              })
            }).catch(err => {
              setTimeout(hide, 1)
